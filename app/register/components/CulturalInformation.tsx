...
export function CulturalInformation({ onPrev }: CulturalInformationProps) {
  ...
  const handleSelect = (
    category: "hobbies" | "movieGenres" | "seriesGenres",
    item: string
  ) => {
  ...
  const getTranslatedOption = (
    category: keyof typeof CULTURAL_KEYS,
    key: string
  ) => {
  ...
  const renderMultiSelect = (
    ...
    const availableOptions = CULTURAL_KEYS[category].filter(
      ...
      <div className="space-y-2">
        ...
        <div className="space-y-2">
          <Popover
            ...
            onOpenChange={(open) =>
              setOpenPopover((prev) => ({ ...prev, [category]: open }))
            }
          >
            ...
            <PopoverContent className="w-full p-0 bg-[#1a1f36] border-[#2e3650]">
              <Command>
                <CommandInput
                  placeholder={`${t(`cultural.${category}`)}...`}
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex justify-between">
        <Button
          onClick={onPrev}
          className="bg-[#2a2d3e] hover:bg-[#3a3d4e] text-white"
        >
          {t("buttons.previous")}
        </Button>
        <Button
