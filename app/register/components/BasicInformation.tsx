...
export function BasicInformation({ onNext }: BasicInformationProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isValidPhoneNumber(formData.phone)) {
      t("messages.invalidPhone"); // Exibe uma mensagem de erro
      return;
    }
    // Check if phone number is valid (required)
    if (!formData.phone) {
      t("messages.phoneRequired"); // Custom error message
      return;
    }
      ...
      <div>
        ...
        <div className="flex gap-1 items-center">
          ...
          <p className="text-red-500 text-sm mt-1">
            {t("messages.phoneRequired")} {!isValidPhoneNumber(formData.phone) && "-"}
          </p>
        )}  */}
        ...
        >
          ...
          <SelectContent>
            <SelectItem value="male">
              {t("basicInfo.genderOptions.male")}
            </SelectItem>
            <SelectItem value="female">
              {t("basicInfo.genderOptions.female")}
            </SelectItem>
            <SelectItem value="other">
              {t("basicInfo.genderOptions.other")}
            </SelectItem>
          </SelectContent>
        </Select>
      </div>
